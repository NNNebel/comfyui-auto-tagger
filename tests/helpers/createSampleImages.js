// tests/helpers/createSampleImages.js
// Helper functions to create sample PNG and WebP images with metadata
 ...
 * @param {Array<{keyword: string, text: string}>} textChunks - Array of text chunks to embed
 * @returns {Uint8Array} PNG image buffer
 */
export function createPngWithMetadata(textChunks) {
 ...
 * @param {string} exifData - EXIF data string to embed
 * @returns {Uint8Array} WebP image buffer
 */
export function createWebpWithMetadata(exifData) {
...
function createWebpChunk(type, data) {
  const paddedSize = data.length + (data.length % 2); // Pad to even byte boundary
